...
class DPICalibrationProcedure:
    OHUB_HEARTBEAT_INTERVAL = 0.050
    CALIBRATION_POINT_LIST = target_positions[13]
    ...
    def __init__(self, eyetrackerInterface, calibration_args, allow_escape_in_progress=True):
        ...
        self.screenSize = eyetrackerInterface._display_device.getPizelResolution()
        self.width = self.screenSize[0]
        self.height = self.screenSize[1]
        self._ioKeyboard = None
        self._msg_queue = []
        self._lastCalibrationOk = False
        self._device_config = self.eyetracker.getConfiguration() ##This needs to be implemented in the eyetracker class
        display = self.eyetracker.display_device
        updateSettings(self._device_config.get('calibration'), calibration_args)
        self._calibration_args = self.device_config.get('calibration')
        unit_type = self.getCalibSettng('unit_type')
        ...
        if cal_type in target_position_count:
            num_points = target_postition_count[cal_type]
        ...
        self.window = visual.Window(
            self.screenSize,
            monitor=display.getPsyschopyMonitorName(),
            units=unit_type,
            fullscr=True,
            allowGUI=True,
        ...
        self._lstMsgPumpTime = currentTime()

        self.clearAllEventBuffers()

    def getCalibSettings(self, setting):
    ...
    def clearAllEventBuffers(self):
        self.eyetracker._iohub_server.eventBuffer.clear()
    ...
    def _registerEventMonitors(self):
        ...
        if kbDevice:
            ...
            for event_class_name in kbDevice.__class__.EVENT_CLASS_NAMES:
                eventIDs.append(getattr(EC, converCamelToSnake(event_class_name[:-5], False)))

            self._ioKeyboard = kbDevice
            self._ioKeyboard._addEventListener(self, eventIDs)
        else: print2err('Warning: %s could not connect to Keyboard device for events.' % self.__class__.__name__)
    ...
    def MsgPump(self):
        if currentTime() - self.lastMsgPumpTime > self.IOHUB_HEARTBEAT_INTERVAL:
            if self._eyetracker._iohub_server:
    ...
    def createGraphics(self):
        ...
        def setDefaultCalibrationTarget():
            ...
            innerRadiusObj = layout.Size(innerRadiusPix, unit=unit_type, win=self.window)
            innerRadius = getattr(innerRadiusObj, unit_type)[1]

            self.targetStim = visualTargetStim(
    ...
    if tcolor is None:
        from psuchopy.iohub.util import complement
    ...
    instruction_text = 'Press SPACE to Start Calibration; ESCAPE to Exit.'
    self.textLineStim = visual.TextSTim(self.window, text=instruction_text,
                                        pos=(0,0), height=36,
